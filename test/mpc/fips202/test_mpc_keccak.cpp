...
void run_party(otpqc::MPCParty<> &party, const std::vector<bool> &input_bits, std::vector<bool> &output_bits) {
    ...
    auto result = otpqc::mpc::protocols::gc::circuit::run(circuit, &input_bits);
...
TEST(MPCNetworkTest, ZeroStateTest) {
    ...
    auto shares = otpqc::mpc::sharing::BooleanSharing<mpz_class>::generate_random_shares(number, 1600, 2);
