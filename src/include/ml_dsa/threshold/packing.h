...
void pack_pk(uint8_t pk[DILITHIUM_CRYPTO_PUBLICKEYBYTES], const uint8_t rho[DILITHIUM_SEEDBYTES], const polyveck *t1);
...
void pack_sig(uint8_t sig[DILITHIUM_CRYPTO_BYTES], const uint8_t c[DILITHIUM_CTILDEBYTES], const polyvecl *z, const polyveck *h);

#define unpack_pk DILITHIUM_NAMESPACE(unpack_pk)
void unpack_pk(uint8_t rho[DILITHIUM_SEEDBYTES], polyveck *t1, const uint8_t pk[DILITHIUM_CRYPTO_PUBLICKEYBYTES]);
...
int unpack_sig(uint8_t c[DILITHIUM_CTILDEBYTES], polyvecl *z, polyveck *h, const uint8_t sig[DILITHIUM_CRYPTO_BYTES]);
