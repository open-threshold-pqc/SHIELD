...
class CMPC { public:
	...
	PRP prp;
	CMPC(NetIOMP<nP> * io, ThreadPool * pool, int party, BristolFormat * cf, bool * _delta = nullptr, int ssp = 40) {
		this->party = party;
		this->io = io;
		this->cf = cf;
		this->ssp = ssp;
	...
	CMPC(CMPC&& other) noexcept {
		...
		ANDS_value = other.ANDS_value;
		other.ANDS_value = nullptr;

		preprocess_value = other.preprocess_value;
		other.preprocess_value = nullptr;

		sigma_value = other.sigma_value;
		other.sigma_value = nullptr;

		if (other.party == 1) {
			GTM = other.GTM; other.GTM = nullptr;
			GTK = other.GTK; other.GTK = nullptr;
			GTv = other.GTv; other.GTv = nullptr;
			GT = other.GT;   other.GT = nullptr;
		}

		for (int i = 1; i <= nP; ++i) {
			key[i] = other.key[i];           other.key[i] = nullptr;
			mac[i] = other.mac[i];           other.mac[i] = nullptr;
			ANDS_key[i] = other.ANDS_key[i]; other.ANDS_key[i] = nullptr;
			ANDS_mac[i] = other.ANDS_mac[i]; other.ANDS_mac[i] = nullptr;
			preprocess_mac[i] = other.preprocess_mac[i]; other.preprocess_mac[i] = nullptr;
			preprocess_key[i] = other.preprocess_key[i]; other.preprocess_key[i] = nullptr;
			sigma_mac[i] = other.sigma_mac[i]; other.sigma_mac[i] = nullptr;
			sigma_key[i] = other.sigma_key[i]; other.sigma_key[i] = nullptr;
			eval_labels[i] = other.eval_labels[i]; other.eval_labels[i] = nullptr;
		}

		num_ands = other.num_ands;
		num_in = other.num_in;
		total_pre = other.total_pre;
		ssp = other.ssp;
		party = other.party;
	}
...
//		ret.get();
	...
	void online (bool * input, bool * output, int shared_output = 1) {
		...
		/* With this new parameter, we control whether the output should remain secret shared or not */
		if (shared_output) {
			if (party != 1)
				memcpy(output, value+cf->num_wire - cf->n3, cf->n3);
			else {
