...
template<int ssp>
inline void inProdhelp(block *Ms,  bool * tmp[ssp],  block * MAC, int i) {
	for(int j = 0; j < ssp; ++j)
...
template<int ssp>
inline void inProds(block *Ms,  bool * tmp[ssp], block * MAC, int length) {
...
template<typename T>
inline void joinNclean(vector<future<T>>& res) {
